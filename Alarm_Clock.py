...
root.configure(bg="#F0E68C") # Pastel Khaki

clock_label = tk.Label(root, font=('Arial', 12), bg="#F0E68C") # Pastel Khaki
clock_label.grid(row=0, columnspan=2, pady=(10, 20))

label_hour = tk.Label(root, text="Hour (0-12):", bg="#F0E68C") # Peach Puff
label_hour.grid(row=1, column=0, pady=(0, 5))
entry_hour = tk.Entry(root, bg="#FFDAB9") # Peach Puff
entry_hour.grid(row=1, column=1, pady=(0, 5))

label_minute = tk.Label(root, text="Minute (0-59):", bg="#F0E68C") # Peach Puff
label_minute.grid(row=2, column=0, pady=(0, 5))
entry_minute = tk.Entry(root, bg="#FFDAB9") # Peach Puff
entry_minute.grid(row=2, column=1, pady=(0, 5))

label_am_pm = tk.Label(root, text="AM/PM:", bg="#F0E68C") # Peach Puff
label_am_pm.grid(row=3, column=0, pady=(0, 5))
am_pm_options = ['AM', 'PM']
am_pm_var = tk.StringVar(root)
am_pm_var.set(am_pm_options[0]) # default value
am_pm_menu = tk.OptionMenu(root, am_pm_var, *am_pm_options)
am_pm_menu.configure(bg="#FFDAB9") # Peach Puff
am_pm_menu.grid(row=3, column=1, pady=(0, 5))

label_day = tk.Label(root, text="Day (1-31):", bg="#F0E68C") # Peach Puff
label_day.grid(row=4, column=0, pady=(0, 5))
day_var = tk.StringVar(root)
day_var.set('1') # default value
day_menu = tk.OptionMenu(root, day_var, *[str(i) for i in range(1, 32)])
day_menu.configure(bg="#FFDAB9") # Peach Puff
day_menu.grid(row=4, column=1, pady=(0, 5))

label_month = tk.Label(root, text="Month (1-12):", bg="#F0E68C") # Peach Puff
...
month_menu.configure(bg="#FFDAB9") # Peach Puff
month_menu.grid(row=5, column=1, pady=(0, 5))

label_year = tk.Label(root, text="Year:", bg="#F0E68C") # Peach Puff
label_year.grid(row=6, column=0, pady=(0, 5))
year_var = tk.StringVar(root)
year_var.set(str(datetime.now().year)) # default value is current year
year_menu = tk.OptionMenu(root, year_var, *[str(i) for i in range(datetime.now().year, datetime.now().year + 10)])
year_menu.configure(bg="#FFDAB9") # Peach Puff
year_menu.grid(row=6, column=1, pady=(0, 5))

set_button = tk.Button(root, text="Set Alarm", command=set_alarm, bg="#87CEFA") # Light Sky Blue
